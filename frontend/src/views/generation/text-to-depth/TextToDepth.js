...
const TextToDepth = () => {
  const [rgbImage, updateRgbImage] = useState();
  const [depthImage, updateDepthImage] = useState();
  ...
  const generate = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/model/generate/text-to-depth",
        {
          model_version: model,
          prompt,
      ...
      updateRgbImage(response.data.rgb_image);
      updateDepthImage(response.data.depth_image);
    } catch (error) {
